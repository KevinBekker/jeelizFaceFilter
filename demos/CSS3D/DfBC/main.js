...
function init_scene(spec) {
    ...
    const samplerVideo = GL.getUniformLocation(
        VIDEOSCREENSHADERPROGRAM,
        "samplerVideo"
    );
    UVIDEOTRANSFORMMAT2 = GL.getUniformLocation(
        VIDEOSCREENSHADERPROGRAM,
        "videoTransformMat2"
    );
        ...
        callbackTrack: function (detectState) {
            ...
            if (ISDETECTED) {
                ...
                const tanFOV = Math.tan(
                    (CAMERA.aspect * CAMERA.fov * Math.PI) / 360
                ...
                MOVEMENT.position.multiplyVectors(
                    MOVEMENT.position,
                    CAMERA.scale
                );
